...
BiometricsFingerprint::BiometricsFingerprint() : mClientCallback(nullptr), mDevice(nullptr) {
    ...
    std::string vendor_modules[] = {"fpc", "fpc_fod", "goodix", "goodix_fod", "goodix_fod6"};

    for (const auto& vendor : vendor_modules) {
        mDevice = openHal(vendor.c_str());
        if (!mDevice) {
            ALOGV("Can't open HAL module, class %s", vendor.c_str());
            continue;
        } else {
            ALOGI("Opened fingerprint HAL, class %s", vendor.c_str());
            break;
        }
    }
}
...
xiaomi_fingerprint_device_t* BiometricsFingerprint::openHal(const char* class_name) {
    ...
    if (kVersion != device->version) {
        ...
        ALOGE("Wrong fp version, class %s. expected %d, got %d", class_name, kVersion,
              device->version);
        return nullptr;
    }
